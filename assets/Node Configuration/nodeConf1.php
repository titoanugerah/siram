...
$conn->close();
 ...
 unset($client); 
  ...
  if ($message->topic=="/data2/suhu1"){
    $sql = "INSERT INTO t_suhu (topik, suhu, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'C-01-001')";
  }

  if ($message->topic=="/data2/dew1"){
    $sql = "INSERT INTO t_dew (topik, dew, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'C-01-001')";
  }

  if ($message->topic=="/data2/tanah1"){
    $sql = "INSERT INTO t_tanah (topik, tanah, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'C-01-001')";
  }

  if ($message->topic=="/data2/udara1"){
