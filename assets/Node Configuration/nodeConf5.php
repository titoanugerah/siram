...
$conn->close();
 ...
 unset($client); 
  ...
  if ($message->topic=="/data2/suhu5"){
    $sql = "INSERT INTO t_suhu (topik, suhu, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'T-01-005')";
  }

  if ($message->topic=="/data2/dew5"){
    $sql = "INSERT INTO t_dew (topik, dew, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'T-01-005')";
  }

  if ($message->topic=="/data2/tanah5"){
    $sql = "INSERT INTO t_tanah (topik, tanah, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'T-01-005')";
  }

  if ($message->topic=="/data2/udara5"){
