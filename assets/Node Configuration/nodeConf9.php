...
$conn->close();
 ...
 unset($client); 
  ...
  if ($message->topic=="/data2/suhu9"){
    $sql = "INSERT INTO t_suhu (topik, suhu, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'C-01-009')";
  }

  if ($message->topic=="/data2/dew9"){
    $sql = "INSERT INTO t_dew (topik, dew, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'C-01-009')";
  }

  if ($message->topic=="/data2/tanah9"){
    $sql = "INSERT INTO t_tanah (topik, tanah, waktu, kode_tanaman)
    VALUES ('$message->topic', '$message->payload', NOW(), 'C-01-009')";
  }

  if ($message->topic=="/data2/udara9"){
